...
u16 print_webpage(u8* buf)
{
   ...
   plen = fill_tcp_data_p(buf, plen, PSTR("<center><p>Sent data to ENC28J60 : "));
   sprintf(a,"0x%x",i);
   plen = fill_tcp_data_p(buf, plen, a);
   plen = fill_tcp_data_p(buf, plen, PSTR(" <p><small><a href=\""));
   ...
   plen = fill_tcp_data_p(buf, plen, PSTR("Open103V WEB test</a><p>"));
